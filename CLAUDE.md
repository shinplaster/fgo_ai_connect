...
### 接続ライフサイクル（app/main.py）— ページ駆動の desired-state 調整ループ
`lifespan` は即座に完了してサーバを常時応答可能にし、接続処理は `_lifecycle_loop()` のバックグラウンドタスクで行う（iPhone 未接続・WDA 未設定でも `/api/status` は応答）。**起動時はアイドル（`desired_connected=False`・自動接続しない）**で、ブラウザの「接続」ボタン（`POST /api/connect`）で初回接続する。

`state.desired_connected` を中心とした desired-state 調整: True の間だけ接続＋維持、False に反転したら切断。`/api/connect`/`/api/disconnect` は desired を反転させて `state.wakeup` Event でループを即時起床する（接続は最大 120s+180s かかるので fire-and-forget、状態は `/api/status` ポーリングで表示）。`asyncio.Lock`（`state.lifecycle_lock`）でウォッチドッグと API を直列化（単一 SelectorEventLoop なので Lock で十分）。ループの3状態:
- `not desired` → `wakeup.wait()` でアイドル待機（起動時も这里）。`user_action`/`error` をクリアして「未接続」に。
- `desired and not ready` → lock 内で `_connect_once()`（busy=true）。失敗時はエラーメッセージ基準で keep_tunnel 決定＋backoff（`wakeup.wait(timeout=backoff)` で desired 反転を即検知）。
- `desired and ready` → `_maintain_until_drop()`（健康チェックループを別関数に抽出・`wakeup` で desired 反転を即時検知）。ドロップ時は desired 仍 True なら再接続、False なら teardown。
...
- **ユーザー「切断」クリック時の teardown** → `keep_tunnel=True`（tunnel 保持で高速再接続）。ただし `WdaDeployer.stop(kill=True)` で **runner プロセスを明示 kill** する（下記「runner の明示 kill」参照）ので端末側「Automation running」banner が消える。
...
### runner の明示 kill（app/device/wda.py）—「Automation running」停止
`WdaDeployer.kill_runner()` が `DvtProvider` + `ProcessControl` で `process_identifier_for_bundle_identifier(runner_bundle_id)` から pid を回復し `pc.kill(pid)` する（pymobiledevice3 の `developer dvt kill` と同パターン）。pid は runner 生存中のみ解決可能（死んでいれば 0 → `if pid:` で no-op）。**task キャンセル・`service_provider.close()` の前**に呼ぶ。`DvtProvider` オープン失敗（tunnel 死）・kill 失敗は broad except で fallback（task cancel）し raise しない。`stop(kill=True)` は `kill_runner()` → `_xctrunner_task.cancel()` → `service_provider.close()` の順。

### ユーザー導線の user_action（app/state.py）
`state.user_action` は `None | "unlock" | "connect_usb"`。`_connect_once` が:
- 冒頭の usbmux デバイス存在確認で未接続なら `"connect_usb"` をセットして即 raise（tunnel 起動をスキップ）。
- create_session がロックエラーのとき `"unlock"` をセットして待機（WDA プロセスは起動済み・MJPEG keepalive で延命中なので再起動せずアンロック待ちが正解）。
UI（app.js）は `user_action` をステータス/overlay ヒントの最優先で表示。`/api/connect`/`/api/disconnect` ハンドラと lifecycle ループのアイドル分支で `user_action`/`error` をクリアし、切断時は「未接続」に戻る。

### UI（app/web/）— タップ/スワイプのみ＋接続/切断ボタン
FGO はタップ/スワイプが基本なのでホーム/音量/snapshot ボタン・テキスト送信 UI は削除済み。`index.html` の `<aside class="controls">` は接続/切断ボタン＋ヒントのみ。`app.js` は `desired`/`busy`/`userAction` をポーリングで追跡し、`user_action` ヒント優先表示・ボタン表示制御（切断は接続中も有効・tunnel 起動120s 中も中止可能）・`/stream` が stale なら自動再読み込み（`?r=<ts>` cache-buster・5s クールダウン）で一時停滞を自己回復。ポインタ操作（`pointerDown/Move/Up` → `/api/tap`/`hold`/`swipe`、リップル）は FGO 操作の核心として保持。
...
### 状態
`app/state.py` の `state` シングルトンが `tunnel`/`deployer`/`wda`/`tunnel_info`/`ready`/`error`/`screen` に加え、ライフサイクル状態（`desired_connected`/`busy`/`lifecycle_lock`/`wakeup`/`tunnel_died`/`user_action`）を保持。lifespan が初期化し、API ルート (`routes_status`/`routes_input`/`routes_stream`) が参照する。`routes_status` には `GET /api/status`（`desired`/`busy`/`user_action` を含む）と `POST /api/connect`/`POST /api/disconnect`（fire-and-forget・desired 反転＋wakeup）が同居。
...
Phase 0（スモーク検証）で実機手順を確定中。Phase 1（WDA ビルド自動化）は `scripts/mac_build.md` で手順化済み。Phase 2（バックエンド: tunnel/WDA 管理 + 操作 API）は **tunnel を remote start-tunnel（CoreDevice pair・Python API）に切り替え・実機検証済み**（tunnel 確立 → testmanagerd 経由 WDA 起動 → WDA ready → tunnel 再利用まで確認・testmanagerd 不在問題 解消）。Phase 3（ストリームプロキシ）はコード実装済み・実機 E2E 検証済み（MJPEG 9.6fps・screenshot フォールバック・`/stream` stale 自己回復）。Phase 4（UI）は `app/web/` に実装済み・**ページ駆動接続/切断＋タップ/スワイプのみの UI に仕上げ済み**（実機 E2E 検証済み）。Phase 5（watchdog/再接続/署名期限/フォールバック）は **desired-state 調整ループ（`_lifecycle_loop`）＋ runner 明示 kill（`kill_runner`）＋ user_action 導線（connect_usb/unlock）＋`/stream` stale 自己回復を実装・実機 E2E 検証済み**（5秒kill根本対策は MJPEG keepalive で軽減・health check failure は自動再接続で回復、iOS15+ パスコード要求ロックは根本変更不可で保留）。
