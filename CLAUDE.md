...
### keepalive（autolock リセット）
Face ID 端末は自動ロック「なし」不可（最大5分）→ ロックで WDA セッションが切れる。`_connect_loop` 維持フェーズで `keepalive_interval`（既定240s）ごとに画面中央の 1px 微小スワイプ（`state.wda.swipe()`）を送り autolock タイマーをリセット（`_keepalive()`、`config.keepalive_enabled`）。**5秒kill（OS レベル kill）とは別問題** — keepalive はロック対策であって 5秒kill の対策ではない。
